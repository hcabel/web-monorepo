...
const PeersConnection = new Map<string, PeerConnectionEntry>();
...
export default function RoomLayer(props: RoomLayerProps) {
	...
	function DConOpen(dc: RTCDataChannel, peerId: string) {
	...
	function DConMessage(peerId: string, msg: MessageEvent<string>) {
		// Parse JSON msg
		let parsedMessage: { _id: string; type: string; audio?: boolean; video?: boolean };
		try {
		...
		console.log(`DC_${peerId}:\tMessage Receveived`, parsedMessage.type);

		const peerIndex = Utils.getPeerIndexFrom_Id(parsedMessage._id, _Peers);
		if (peerIndex === -1) {
			// Update Audio status
			console.error(
				`DC_${peerId}:\tFailed to find peerIndex from`,
				parsedMessage._id
			);
			return;
		}

		if (parsedMessage.type === "audioStateChange") {
			// Update Audio status
			const peers = [..._Peers];
			peers[peerIndex].audio = parsedMessage.audio;
			set_Peers(peers);
		} else if (parsedMessage.type === "videoStateChange") {
			// Update Video status
			const peers = [..._Peers];
			peers[peerIndex].video = parsedMessage.video;
			set_Peers(peers);
		}
	}

	function DConClose(peerId: string) {
		console.log(`DC_${peerId}:\tDisconnected`);
	}

	function DConError(dc: RTCDataChannel, event: Event, peerId: string) {
		console.error(`DC_${peerId}:\tError`, event);
		dc.close();
	}

	function initDCFunctions(dataChannel: RTCDataChannel, peerId: string) {
	...
	function sendMessageToEveryoneInTheRoom(msg: string) {
	...
	function sendStreamsToPeers(connection: RTCPeerConnection | undefined, stream: MediaStream) {
	...
	function onIceCandidate(e: RTCPeerConnectionIceEvent, peerId: string) {
		if (!e.candidate) {
	...
	function onTrack(event: RTCTrackEvent, peerId: string) {
		// When you receive streams from the peer

		console.log(`WebRTC:\tYou received STREAM from Client_${peerId}`);
		const video = document.getElementById(`VideoStream_${peerId}`) as HTMLVideoElement | null;
		if (video) {
	...
	function onNegotiationNeeded(connection: RTCPeerConnection, peerId: string) {
	...
	function setRemoteThenLocalDescription(connection: RTCPeerConnection, offer: RTCSessionDescriptionInit, peerId: string) {
		...
		connection
			...
			.then(() =>
				console.log(
					`WebRTC:\tClient_${peerId}:\tRemote description set`
				)
			);
	...
	function sendAnswerBasedOffer(offer: RTCSessionDescriptionInit, peerId: string) {
		console.log(
			`WebRTC:\t>>> New peer connection with: Client_${peerId} <<<`
		);
		...
		newConnection.onnegotiationneeded = () =>
			onNegotiationNeeded(newConnection, peerId);
	...
	function createNewPeerConnection(peerId: string) {
		console.log(
	...
	function RTCMessageDispatcher(msg: RTCSignalMessage) {
		if (msg.type === "Offer") {
			...
			if (PeersConnection.has(msg.from)) {
				...
				console.log(
					`WebRTC:\t>>> Client_${msg.from} send you his Offer <<<`
				);
				const currentConnection = PeersConnection.get(msg.from);
				if (currentConnection && msg.offer) {
					setRemoteThenLocalDescription(
		...
		if (!connection) {
			throw Error(
				`You receive a RTC message from a undefined peer:\t${msg.type}`
			);
		}
		...
		if (msg.type === "Answer") {
			...
			if (msg.answer) {
				connection.PC.setRemoteDescription(msg.answer).then(() =>
					console.log(
						`WebRTC:\tClient_${msg.from} Remote description set`
					)
				);
			}
			...
			if (!msg.iceCandidate) {
				console.warn(
					`WebRTC:\tClient_${msg.from}:\tSend you a not valid ICE candidate`
				);
				return;
			}
			...
			connection.PC.addIceCandidate(ice).then(() =>
				console.log(
					`WebRTC:\tAdd new ICE from Client_${msg.from}\t${ice.type}`
				)
			);
		}
	}

	async function onRoomSetup(msg: { peers: Peer[] }) {
		set_Peers(msg.peers);

		const video = document.getElementById(`LocalStream`) as
			| HTMLVideoElement
			| null;
		if (video) {
	...
	function toggleVideo() {
		props.onChangeVideoStatus(!props.video).then((newStream) => {
			if (!props.video && newStream) {
	...
	function sendJoinRequestResponce(approved: boolean, id: string) {
		set_PendingInvitation([
			..._PendingInvitation.filter((value) => {
				return value._id !== id;
			}),
		]);
	...
	async function WSonMessage(msg: MessageEvent<string>) {
		...
		console.log(`--> SS: ${parsedMessage.type}`);

		if (parsedMessage.type === "RoomSetupCallback") {
			// This will initialise the room from your side
			onRoomSetup(parsedMessage);
		} else if (parsedMessage.type === "clientJoin") {
			// Add to the peer map (we dont create peerConnection because he will ask us to connect)
			set_Peers([..._Peers, parsedMessage.newPeer]);
		} else if (parsedMessage.type === "clientLeave") {
			if (parsedMessage.role === "Client" || parsedMessage.role === "Owner") {
				// If it was a client or the owner of the room, close is PeerConnection
				const peerConnection = PeersConnection.get(parsedMessage.from);
				if (peerConnection) {
					if (
						peerConnection.PC &&
					...
					if (
						peerConnection.DC &&
						peerConnection.DC.readyState === "open"
					) {
						peerConnection.DC.close();
					}
					PeersConnection.delete(parsedMessage.from);
				}

				const newPeers = _Peers.filter((peer) => {
					return peer._id !== parsedMessage.from;
				});
				set_Peers(newPeers);
			} else if (
				parsedMessage.role === "Pending" &&
				_PendingInvitation.length > 0
			) {
				// If it was a client who was waiting for joining in the room, remove is invitation
				set_PendingInvitation([
					..._PendingInvitation.filter((value) => {
						return value._id !== parsedMessage.from;
					}),
				]);
			}
		} else if (parsedMessage.type === "OwnershipReceived") {
			console.log("You've been promoted");
			// TODO: Show the admin panel or something
		} else if (parsedMessage.type === "JoinRequestReceived") {
			console.log("Someone want to join the room:", parsedMessage.from);
			...
			set_PendingInvitation([
				..._PendingInvitation,
				{ name: parsedMessage.name, _id: parsedMessage.from },
			]);
		} else if (Utils.rtc.isRTCMessage(parsedMessage.type)) {
			// msg.type === Offer | Answer | IceCandidate
			RTCMessageDispatcher(parsedMessage);
		} else {
			console.error(`Msg dropped because type ${parsedMessage.type} is unknown`);
		}
	}
			...
			<div
				...
				style={{
					gridTemplateColumns: `${"auto ".repeat(
						numberOfColumns[_Peers.length as keyof typeof numberOfColumns] || 1
					)}`,
				}}
			>
				{_Peers.map((peer, index) =>
					props.selfId && peer._id === props.selfId ? (
						<PeerVideo
							...
							index={index}
							id="LocalStream"
							stream={window.localStream || undefined}
							name={peer.name}
							audio={props.audio}
							video={props.video}
							muted
