...
export default function RoomPage() {
	...
	async function InitStreams(audio, video) {
		...
		if (!navigator.mediaDevices) {
			alert(
				"This site is untrusted we cant access to the camera/or and microphone !"
			);
			navigate("/");
			return;
		}
		...
		if (audio) {
			await navigator.mediaDevices
				...
				.then((newAudioStream) => {
					...
					if (localVideo) {
						streamResult = Utils.media.combineStream(
							streamResult,
							newAudioStream
						); // currentStream + newAudioTracks
